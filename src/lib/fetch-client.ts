...
import { handleRequestError as handleRequestErrorUtil } from "./fetch-client/error-handler";
...
export class FetchClient {
   ...
   * - Sets Content-Type to application/json if needed
  ...
  private createDefaultHeaders(
    ...
    const { customHeaders = {}, includeContentType = true } = options;
  ...
  private prepareRequestHeaders(
    ...
    return this.createDefaultHeaders({
      customHeaders,
      includeContentType: method !== "GET" && body !== undefined,
    });
  }
   ...
   *
   * Purpose: Converts body to string if needed
   *
   * @param body - Request body
   * @returns Prepared body string or undefined
   */
  private prepareRequestBody(body?: unknown): string | undefined {
    if (body === undefined) {
      return undefined;
    }
  ...
  async request<TResponse = unknown>(
    ...
    try {
      ...
      // - If stream === true: force streaming
      // - If stream === false: force non-streaming (ignore auto-detection)
      // - If stream === undefined: auto-detect based on response
      const shouldStream =
        stream === true ||
        (stream === undefined && shouldStreamResponse(response));
